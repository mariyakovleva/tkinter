...
canv.pack()
def draw_20_cirle_count_red_and_green(event):
    count_red=0
    count_green=0
    for i in range(20):
        R=randint(10,40)
...
def draw_while_not_red_and_green(event):
    for i in range(20):
        R=randint(10,40)
        x=randint(R,400-R)
        y=randint(R,400-R)
...
btn1.pack(side=LEFT)
btn1.bind('<1>',draw_20_cirle_count_red_and_green)

btn2=Button(fr,text ='Draw while not red and green')
btn2.pack(side =LEFT, padx=10)
btn2.bind('<1>',draw_while_not_red_and_green)
